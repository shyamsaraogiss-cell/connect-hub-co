...
import { getCustomerSafeWorkflow } from "../services/commercialWorkflow.service";
...
export async function trackUniversalRequest(req: AuthenticatedRequest, res: Response) {
  ...
  return res.json({ ...record, commercialWorkflow: await getCustomerSafeWorkflow(record.referenceId, record) });
}

...
// FIXED SHELL
// =======================================
export function PublicHeroShell({children,fullWidth = false,showSidebar = true}:{children:ReactNode;fullWidth?:boolean;showSidebar?:boolean}){return <div className={styles.shell}><div className={`${styles.layout} ${fullWidth ? styles.fullWidthLayout : ''} ${showSidebar ? '' : styles.withoutSidebar}`}>{/* FIXED SITARAM SHELL — DO NOT MODIFY */}{showSidebar ? <Suspense fallback={<div className={styles.sidebar} aria-hidden="true" />}><PublicHeroSidebar/></Suspense> : null}<div className={styles.content}>{children}</div></div></div>}

...
import { getAllPayments } from '@/services/payment.api';
...
export function CustomerDashboard() {
  ...
  const customerPayments = getAllPayments();
    ...
    <div className="min-h-screen bg-stone-50 text-stone-900">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 space-y-6">
        ...
        {activeTab === 'ai-history' && (
          <div className="rounded-2xl border border-teal-900/20 bg-white p-6 shadow-sm space-y-4">
            <h3 className="font-serif text-2xl text-teal-900">GenZ Ritual AI Conversation History</h3>
        ...
        {activeTab === 'payments' && (
          <div className="rounded-2xl border border-teal-900/20 bg-white p-6 shadow-sm space-y-4">
            <h3 className="font-serif text-2xl text-teal-900">Payment & Transaction Receipts</h3>
            <div className="space-y-3 text-xs">
              {customerPayments.map((txn) => (
                <div key={txn.id} className="flex items-center justify-between rounded-xl border border-stone-200 p-4">
                  <div>
                    <div className="font-bold text-teal-900">{txn.id} — {txn.category.toUpperCase()}</div>
                    <div className="text-stone-500">{txn.paymentGateway} | {new Date(txn.createdAt).toLocaleDateString()}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-stone-900">₹{txn.amount.toLocaleString('en-IN')}</div>
                    <span className="rounded bg-teal-100 px-2 py-0.5 text-[10px] font-bold text-teal-800">{txn.status}</span>
                  </div>
                </div>
              ))}
            </div>

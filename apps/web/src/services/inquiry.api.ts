...
export async function submitGeneralInquiry(data: GeneralInquiryInput): Promise<SubmitInquiryResult> {
  const hash = `general:${data.fullName.trim().toLowerCase()}:${data.phone.trim()}`;
  if (submittedInquiries.has(hash)) {
    return { success: false, message: 'A similar inquiry has already been submitted recently. Our team will contact you shortly.' };
  }
  ...
  const record = await createUniversalRequest({
    ...
    title: `General Inquiry: ${data.inquiryCategory}`,
  ...
  return { success: true, inquiryId: record.referenceId };
}
